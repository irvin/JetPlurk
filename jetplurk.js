...
"<div id='sendPlurk'></div><msgs><msg></msg></msgs><div id='loadmore' class='button'><a href='#'>Load more</a></div>" +
"</div></body>";
...
function reFreshPlurk() {
	...
	$.ajax({
		...
		success: function(json) {
			...
			$(sliderObj.contentDocument).find("div#sendPlurk").fadeOut('medium', function() {
				$(sliderObj.contentDocument).find("div#sendPlurk").remove();			
				addSendPlurk();
			});
...
function addSendPlurk() {
	var content = "<div id=\'sendPlurk\'><form id=\'sendform\'><textarea name=\'content\' rows=\'1\'></textarea><input id=\'send_button\' type=\'submit\' value=\'Plurk\' /></form></div>";
	$(sliderObj.contentDocument).find('div#banner').after(content);
	var sendFormObj = $(sliderObj.contentDocument).find('form#sendform');

	$(sendFormObj).find("input#send_button").click(function(event) {
		...
		if (response_txt != "") {
			$.ajax({
				...
				error: function(xhr, textStatus, errorThrown) {
					console.log('Plurk error: ' + xhr.status + ' ' + textStatus + ' ' + errorThrown);
				}
			})
		}
