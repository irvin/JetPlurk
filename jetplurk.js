				...
				if (selectPlurkRead == 'unread'){
					...
					$.ajax({
						...
						success: function(json){
							console.log(json);
		    						$(sliderObj.contentDocument).find("#"+selectPlurkID).find("content").removeClass("unread");
		    						//$(sliderObj.contentDocument).find("#"+selectPlurkID).find("content").css({'font-size' : 'large'});
...
function reFreshPlurk() {
	...
	$.ajax({
		...
		success: function(json){
			...
			//$(sliderObj.contentDocument).find("msg").fadeOut('slow').remove();
