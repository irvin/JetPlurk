...
function reFreshPlurk() {
	...
	$.ajax({
		...
		success: function(json) {
			...
			var content = "<div id='usermeta'><img id='useravatar' src='" + avatarurl + "' /><span class='displayname'>" + user_displayname + "</span> <span class='karma'>Karma:" + jsObject.user_info.karma + "</span></div>";
...
function ShowNewPlurk(jsObject) {
	...
	$(jsObject.plurks).each(function(i) {
		...
		var content = '<msg id=\"' + jsObject.plurks[i].plurk_id + '\"> ' + owner_display_name + ' ';
		if (qualifier != '') {
			content += '[' + qualifier + '] ';
		}
		...
		content += jsObject.plurks[i].content + '</content><br><span class=\"meta\"><timestamp>' + timestr + ' </timestamp><a class=\"permalink\" href=\"http://www.plurk.com/m/p/' + premalink + '\" target=\"_blank\">link</a>';
	...
	$(sliderObj.contentDocument).find("msg").click(function() {
		...
		if ($(clickMsg).find("responses").html() == null) {
			...
			// Show response form
			var content = '<responseform id=\"responseform\" class=\"' + selectPlurkID + '\"><form id=\"response\"><textarea name=\"content\" rows="1"></textarea>' + '<input id=\"response_button\" type=\"submit\" value=\"Reponse\" /></form></responseform>';
