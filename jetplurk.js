...
jetpack.slideBar.append({
			...
			onReady : function(slider) {
				...
				$(sliderObj.contentDocument).find('#loadmore').click(
						function() {
							loadMorePlurk();
						})
			},
...
function reFreshPlurk() {
	...
	$.ajax({
		...
		success : function(json) {
			...
			if ((jsObject.user_info.has_profile_image == 1)
				...
				avatarurl = 'http://avatars.plurk.com/'
						+ jsObject.user_info.uid + '-medium.gif';
			} else if ((jsObject.user_info.has_profile_image == 1)
					&& (jsObject.user_info.avatar != null)) {
				avatarurl = 'http://avatars.plurk.com/'
						+ jsObject.user_info.uid + '-medium'
						+ jsObject.user_info.avatar + '.gif';
		...
		error : function(xhr, textStatus, errorThrown) {
			// Login error
			console.log('Login error: ' + xhr.status + ' ' + textStatus + ' '
					+ errorThrown);
		}
...
function loadMorePlurk() {
	...
	$.ajax({
				...
				success : function(json) {
					...
					ShowNewPlurk(jsObject);
					console.log('JetPlurk Load More: NewOffset ' + NewOffset
							+ ' OldOffset ' + OldOffset + ' ReadOffset '
							+ ReadOffset);
				},
				error : function(xhr, textStatus, errorThrown) {
					// Login error
					console.log('Load More error: ' + xhr.status + ' '
...
function ShowNewPlurk(jsObject) {
	...
	$(jsObject.plurks).each(function(i) {
		...
		content += jsObject.plurks[i].content
				...
				+ timestr
				+ ' </timestamp><a class=\"permalink\" href=\"http://www.plurk.com/m/p/'
				+ premalink + '\" target=\"_blank\">link</a>';
	...
	$(sliderObj.contentDocument).find("msg").hover(function() {
		...
		// '] Plurk time: ' + selectPlurkTimestamp +
		// Date.parse(selectPlurkTimestamp) + ' ReadOffset ' + ReadOffset);

		if ((selectPlurkRead == 'unread')
				|| (selectPlurkRead == 'unreadresponse')) {
	...
	$(sliderObj.contentDocument).find("msg").click(function() {
		...
		if ($(clickMsg).find("responses").html() == null) {
			...
			var content = '<responseform id=\"responseform\" class=\"'
					+ selectPlurkID
					+ '\"><form id=\"response\"><textarea name=\"content\" rows="1"></textarea>'
				...
				if (response_txt != "") {
					...
					$.ajax({
						...
						success : function(json) {
							...
							$(clickMsg).find("form#response").get(0).reset();	
